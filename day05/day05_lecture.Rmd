...
mysleep.long <- gather(mysleep.wide, "treatment", "time", pre:post) # or c("pre", "post") or c(pre, post), c(2, 3)
mysleep.long
```
...
who.long <- separate(who.long, stuff, sep = "_", c("newold", "diagnosis", "gender.age"))
summary(who.long)
head(who.long)
...
ff.summaries <- summarize(group_by(myff.long, treatment, flavor), mean = mean(rating, na.rm = TRUE), std.dev = sd(rating, na.rm = TRUE), N = length(rating[!is.na(rating)]))
ff.summaries
